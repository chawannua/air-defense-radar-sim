...
## Release History & Version Evolution (v1.0.0 → v1.8.0)
...
### Version Evolution: `v1.0.0` ➔ `v1.1.0` ➔ `v1.1.1` ➔ `v1.2.0` ➔ `v1.3.0` ➔ `v1.3.1` ➔ `v1.3.2` ➔ `v1.4.0` ➔ `v1.4.1` ➔ `v1.5.0` ➔ `v1.5.1` ➔ `v1.6.0` ➔ `v1.7.0` ➔ `v1.8.0`
...
- **Cursor-Anchored Zoom:** Screen position is `CX + x_km * zoom` with `CX` at Bangkok, so zoom alone magnified about Bangkok. `zoom_anchor()` now moves the camera so the world point under the cursor stays under it — asserted algebraically over 36 camera, cursor and zoom-step combinations; the residual is integer truncation of the camera offset, under one pixel at any zoom.
- **Quieter In Player Mode:** `threat_ceiling_scale` halves every hourly ceiling for a player and raises it 50% for a spectator, resolved once per `CommandCenter`. In Player mode: **1 ARM, 2 TBM, 1 ICBM** an hour, against 8 ARM and 307 ballistic launches an hour measured on v1.7.0.
- **No Empty Battle Stations:** A wave used to log its red banner and burn its cooldown before drawing contacts, so once the ceilings bound, **84% of waves announced an attack that never arrived**. Waves are drawn first and abandoned in silence if the pool is capped out. **0 of 59 empty**, at 6.6 waves an hour.
- **Alerts Tell The Truth:** The SEAD warning used to fire with no anti-radiation missile in the wave at all (**5 of 10 alerts**), because the pool carried a stray cruise missile and the banner was gated on the theme rather than the draw. Mass-attack banners fired over a single contact (**49 of 163**). Both are now gated on what actually launched: 0 false SEAD alerts and 0 thin alerts over 12 simulated hours.
...
- **Application Icon:** PPI radar-scope icon at 256/128/64/48/32/16 px drawn from the simulation's own phosphor palette, replacing PyInstaller's default.
- **Third-Party Attribution:** `THIRD_PARTY_NOTICES.md` covers all 78 bundled native libraries and includes the LGPL-2.1 §6 relinking offer for pygame.
- **Licence Ships With The Binary:** `LICENSE` and `THIRD_PARTY_NOTICES.md` are now bundled inside the executable, not just the repository.
- **Antivirus Posture:** UPX compression disabled — packed entry points are a common AV heuristic.
